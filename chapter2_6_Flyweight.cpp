...
struct SharedState
{
    ...
    SharedState() = default; // 必须提供默认构造函数,不然会报错
...
public:
    Flyweight() = default; // 必须提供默认构造函数,不然会报错
    Flyweight(const SharedState sharedState) : m_sharedstate(sharedState) {}
    void operation(UniqueState uniqueState) const // 使用的时候,使用外部状态作为参数
    {
...
public:
    CarInfoContext(Flyweight *flyWeight, const UniqueState *uniqueState)
        : m_flyWeight(flyWeight), m_uniqueState(*uniqueState) {}
    void operation()
    {
...
int main()
{
    ...
    database.addCarToPoliceDatabase(factory, "xrk", "粤xxxxx", "奔驰", "GLC", "白色");
    factory.listFlyWeights();
    database.addCarToPoliceDatabase(factory, "xrk", "粤yyyyy", "比亚迪", "秦", "白色");
    database.addCarToPoliceDatabase(factory, "xrk", "粤zzzzz", "奔驰", "GLC", "白色");
    factory.listFlyWeights();
